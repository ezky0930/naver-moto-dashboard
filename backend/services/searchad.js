...
// 인증: HMAC-SHA256(timestamp.method.uri, UTF-8 secretKey) → Base64
...
// 광고 API는 클라우드(Vercel 등) IP를 invalid-apikey(403)로 차단한다.
// 한 번 403을 받으면 프로세스 생존 동안 호출을 중단해 매 요청 헛호출을 막는다.
let adBlocked = false

export function isAdConfigured() {
  return !adBlocked && !!(process.env.NAVER_AD_API_KEY && process.env.NAVER_AD_SECRET_KEY && process.env.NAVER_AD_CUSTOMER_ID)
}

function markBlockedIf403(err) {
  if (err.response?.status === 403) {
    adBlocked = true
    console.warn('[searchad] 403 invalid-apikey — 이 서버 IP에서 광고 API 차단됨, 이후 호출 생략')
    return true
  }
  return false
}
...
export async function discoverRelatedKeywords(seedKeywords = ['오토바이헬멧', '오토바이용품', '바이크용품']) {
  ...
  for (const seed of seedKeywords) {
    try {
      ...
      console.error(`[searchad/discover] "${seed}" 오류:`, err.response?.data ?? err.message)
      if (markBlockedIf403(err)) break // IP 차단이면 나머지 씨앗도 모두 실패 — 즉시 중단
    }
  }
